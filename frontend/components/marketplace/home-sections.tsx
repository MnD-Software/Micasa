...
import { ArrowLeft, ArrowRight, CalendarRange, ClipboardList, Compass, ConciergeBell } from "lucide-react";
...
export function HomeSections() {
  ...
  return (
    <main className="mx-auto max-w-[1820px] px-5 pb-28 pt-5 sm:px-6 sm:py-6 lg:px-10">
      <section id="services" className="my-5 grid gap-4 overflow-hidden rounded-[24px] border border-white bg-[#12302c] p-3 text-white shadow-luxe ring-1 ring-brand-line/70 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="relative min-h-[280px] overflow-hidden rounded-[20px] sm:min-h-[360px]">
          <Image
          ...
          <div className="absolute bottom-0 p-5 sm:p-7">
            <p className="text-xs font-bold uppercase text-white/72">Signature stay</p>
            <h2 className="mt-3 max-w-xl font-serif text-3xl font-bold leading-tight sm:text-5xl">{flagship.title}</h2>
        ...
        <div className="grid content-between gap-4 p-2 sm:p-4">
          <div>
            ...
            <h2 className="mt-4 font-serif text-3xl font-bold leading-tight sm:text-5xl">
              Less scrolling. Better matches. Clearer choices.
            </h2>
            <p className="mt-4 text-sm leading-7 text-white/70">
              The homepage now works like a stay board: quick filters, high-signal cards, and support paths for the practical decisions guests actually make.
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              [Compass, "Area-fit", "Beach, mall, pool and commute context"],
              [CalendarRange, "Trip-fit", "Short breaks, group stays and work trips"],
              [ClipboardList, "Clear details", "Guest count, bedrooms, fees and rules"],
              [ConciergeBell, "Local add-ons", "Chef, transfers and stay support"]
            ].map(([Icon, title, text]) => (
